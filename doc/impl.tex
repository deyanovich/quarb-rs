...
\texttt{quarb-text} & The shared text-level model — the relational
  pattern for documents: producers lower their format into one
  ...
  of the subtree. Plain text (\texttt{.txt}, blank-line
  paragraphs) mounts here by default; \texttt{text:} forces the
  reading for html/md. Depends on \texttt{quarb}. \\
\texttt{quarb-text-html} & The HTML producer for the text level:
  ...
  attribution. Depends on \texttt{quarb-text} and
  \texttt{scraper}. \\
\texttt{quarb-text-markdown} & The Markdown producer for the text
  level, mapping pulldown-cmark events directly (no HTML
