...
\texttt{quarb-gsheet} & Google Sheets on the
  ...
  fetched from the Sheets API. Auth: the API rejects
...
\texttt{quarb-pdf} & PDF internals as the object graph: indirect
  objects are nodes named by their declared \texttt{/Type},
  scalar dictionary entries are properties, references are edges
  labeled by their key (\texttt{->Kids}, \texttt{<-Parent}; array
  references hoist to the array's owner), with the reverse index
  built at load. Depends on \texttt{quarb} and \texttt{lopdf}. \\
\texttt{quarb-text} & The shared text-level model — the relational
  pattern for documents: the per-format crates lower their format into one
  block-event vocabulary (the atrep shape: kind, taxis, lemma,
  body, hypograph), and this crate alone implements the adapter.
  Sections with explicit title (\texttt{::lemma}) + body are
  derived from the flat heading stream by the outline rule;
  paragraphs, blockquotes (attribution as \texttt{::hypograph}),
  lists, and verbatim blocks (\texttt{::::lang}) complete the
  vocabulary; tables denormalize into nested lists carrying the
  \texttt{<table>} trait. Bare \texttt{::} is the flattened prose
  of the subtree. The apparatus is first class (litogramma's
  model): \texttt{footnote}/\texttt{endnote} callouts and bodies
  share their family name, resolve by (family, onym) into
  \texttt{->footnote}/\texttt{->endnote} edges, and carry
  \texttt{<deixis>} (the callout), \texttt{<note>} (the body),
  and \texttt{<dangling>} traits; the \texttt{aside} family
  (anchored margin content) rides the same machinery without
  \texttt{<note>}, and margin-spelled footnotes carry
  \texttt{::::form}; \texttt{index-mark} nodes expose
  declared index marks (\texttt{::term}) invisible to the prose. Plain text (\texttt{.txt}, blank-line
  paragraphs) mounts here by default; \texttt{text:} forces the
  reading for html/md. Depends on \texttt{quarb}. \\
\texttt{quarb-text-docx} & The Word text-level adapter,
  from declared structure only: outline levels resolved through
  the style chain (never style names), the accepted view of
  tracked changes, \texttt{w:tblHeader} header rows, the closed
  cited quote-style set, both note parts
  (\texttt{word/footnotes.xml}, \texttt{word/endnotes.xml}), and
  \texttt{XE} field instructions as index marks. Depends on
  \texttt{quarb-text}, \texttt{zip}, and \texttt{quick-xml}. \\
\texttt{quarb-text-epub} & The EPUB text-level adapter: container to
  rootfile to spine, chapters through the HTML adapter in spine
  order (\texttt{linear="no"} excluded); the EPUB note vocabulary
  arrives via \texttt{quarb-text-html}. Depends on
  \texttt{quarb-text}, \texttt{quarb-text-html}, and
  \texttt{zip}. \\
\texttt{quarb-text-html} & The HTML text-level adapter:
  soup dropped (nav, chrome, scripts, forms, media), wrappers
  transparent, headings flat into the shared derivation, trailing
  \texttt{cite}/\texttt{footer}/\texttt{figcaption} as quote
  attribution; notes only from the declared vocabularies
  (\texttt{epub:type} noteref/footnote/endnote/rearnote/%
  marginalia, DPUB-ARIA \texttt{role} doc-*), endnote words
  landing in the endnote family, marginalia in the aside
  family. Depends on \texttt{quarb-text} and
  \texttt{scraper}. \\
\texttt{quarb-text-koine} & The koine adapter — the hub's
  first rung: a koine/litogramma document (parsed by the
  \texttt{atrep} crate, settled by \texttt{kanonizo::tasso})
  lowers into the shared block vocabulary through its
  dialektos-resolved sim names — enclosing headings by nesting
  depth, both note families and asides by name, index endos as
  marks, stichoi tables, refs keeping their key as written.
  Foreign formats arrive through atrep's endomorphosis
  importers (\texttt{atrep::endo}) into their mirror dialects —
  Markdown, HTML, reStructuredText, Org, djot — read by the same
  name-keyed mapping (flat \texttt{heading-N} endos, flat item
  runs wrapped, rst footnotes into the shared apparatus). Serves
  \texttt{koine:} (and \texttt{text:} on
  \texttt{.atd}/\texttt{.atk}, where the routes converge);
  \texttt{.xml} dispatches by declared identity (root
  namespace, DOCTYPE public id, unambiguous root element) into
  the TEI/DocBook/JATS/USX/OSIS importers, \texttt{?format=}
  forcing any format; a bare \texttt{<article>} and everything
  nothing imports yet — LaTeX included — refuse naming the
  working spelling. Depends on \texttt{quarb-text} and
  \texttt{atrep}. \\
\texttt{quarb-text-latex} & The LaTeX text-level adapter: the declared
  structural subset of a \texttt{.tex} source — declared
  sectioning commands, footnotes (the at-callout convention
  normalized to document-end bodies), the endnotes package on
  its own counter, Tufte sidenotes into the footnote family
  (\texttt{::::form}), \texttt{\textbackslash marginpar} /
  \texttt{\textbackslash marginnote} as asides,
  \texttt{\textbackslash index} marks,
  quote/quotation, itemize/enumerate, verbatim; prose by drop
  list and unwrap-by-default, definition argument trains consumed
  whole. Depends on \texttt{quarb-text}. \\
\texttt{quarb-text-markdown} & The Markdown text-level
  adapter, mapping pulldown-cmark events directly (no HTML
  round-trip): heading levels, list ordinals, fence languages,
  and pipe tables come from the source. Depends on
  \texttt{quarb-text} and \texttt{pulldown-cmark}. \\
\texttt{quarb-text-pdf} & The PDF text-level adapter — the reader's model
  from the two declared rungs: the \texttt{/Outlines} tree
  (titles as \texttt{::lemma}, destinations bounding each
  section's extent) and exact-baseline line geometry
  (bit-for-bit equal \texttt{y}, \texttt{::::page}/\texttt{::::y}
  metadata; ToUnicode CMaps decoded, one stated kern constant for
  word gaps). No \texttt{paragraph} is ever minted. Depends on
  \texttt{quarb-text}, \texttt{quarb-pdf}, and \texttt{lopdf}. \\
...
\texttt{quarb-mssql} & The SQL Server catalog driver:
  \texttt{INFORMATION\_SCHEMA} plus \texttt{sys.foreign\_key\_columns}
  (parallel column ids keep composite keys paired), over
  \texttt{tiberius} (pure-Rust TDS, internal current-thread
  runtime); \texttt{bit}/integer/float/character families
  native, everything else \texttt{CAST(... AS NVARCHAR(MAX))}.
  Target: \texttt{mssql://USER:PASS@HOST[:PORT]/DATABASE}.
  Depends on \texttt{quarb-relational}. \\
\texttt{quarb-oracle} & The Oracle catalog driver: the
  \texttt{USER\_*} dictionary views over the \texttt{oracle}
  crate (ODPI-C, synchronous); \texttt{NUMBER(p,0)} integer,
  \texttt{BINARY\_FLOAT}/\texttt{DOUBLE} float, character
  family text, \texttt{DATE}/\texttt{TIMESTAMP} through an
  ISO-8601 \texttt{TO\_CHAR} into the temporal regime,
  everything else \texttt{TO\_CHAR}. Target:
  \texttt{oracle://USER:PASS@HOST[:PORT]/SERVICE}. Depends on
  \texttt{quarb-relational}. \\
\texttt{quarb-ldap} & The LDAP directory driver: the DIT
  \emph{is} the arbor — the base DN is the root, an entry's
  children are the entries one level below (RDN names), its
  attributes are properties (multi-valued as lists),
  \texttt{objectClass} values are traits. DN-valued attributes
  (\texttt{member}, \texttt{manager}, \dots) are native
  references — \texttt{\textasciitilde>} follows, \texttt{->}
  enumerates, \texttt{<-member} finds the groups by a filtered
  search. Over the \texttt{ldap3} sync client, anonymous or
  simple bind. Target:
  \texttt{ldap[s]://[USER:PASS@]HOST[:PORT]/BASE\_DN}. \\
\texttt{quarb-cbor} & The CBOR (RFC~8949) adapter, the JSON
  mapping extended: non-text map keys name nodes by canonical
  rendering, byte strings are hex scalars (\texttt{::::length}
  is the raw byte count), the standard time tags (0, 1) decode
  to instants, other tags are transparent with the number at
  \texttt{::::tag}. Traits carry the CBOR type. Binary, by the
  \texttt{.cbor} extension. Depends on \texttt{ciborium}. \\
...
\texttt{quarb-xpath} & The XPath~1.0 bridge, both directions:
  ...
  explicit refusals and divergence notes both ways. Binaries
...
\texttt{quarb-tree-sitter} & Source code as its literal syntax
  tree — the syntax level (né \texttt{quarb-code}): node kinds
  as edge names (\texttt{//function\_item}), tree-sitter
  \emph{fields} as properties (\texttt{::name} on a function is
  its name), source text as the value, \texttt{::::start-line} /
  \texttt{::::kind} / \texttt{::::field} metadata; named nodes
  only. Rust / Python / JavaScript / C by extension,
  compile-time grammar set. Grafts under composition —
  \texttt{--graft} queries every function in a directory tree
  at once. \\
\texttt{quarb-code} & Source code at the code level:
  declaration identifiers are node names
  (\texttt{/lexer/lex}), anonymous constructs keep a
  normalized cross-language vocabulary (\texttt{if},
  \texttt{switch}, \texttt{for}, \texttt{call}), everything
  else dissolves (children hoist). Curated traits
  (\texttt{<function>}, \texttt{<type>}, \texttt{<loop>});
  \texttt{::signature} / \texttt{::doc} / \texttt{::callee};
  raw backend kind only at \texttt{::::kind} (aliased, closed
  surface); \texttt{->definition} file-local crosslinks.
  Opt-in via the \texttt{code:} target prefix;
  \texttt{code:DIR} grafts every supported source leaf —
  filesystem into declarations, one namespace. \\
\texttt{quarb-compose} & Adapter \emph{composition}: wrap any
  ...
  archives, \texttt{--graft} for directories. \\
...
\texttt{quarb-mongodb} & MongoDB: collections hold BSON
  documents, named by \texttt{\_id} (ObjectIds as hex); embedded
  documents and arrays descend as children. DBRef fields
  (\texttt{\{\$ref, \$id\}}) are native references
  (\texttt{\textasciitilde>}/\texttt{->} schema-free). BSON dates
  arrive as instants; ObjectId and Decimal128 as strings. Lazy
  per collection (one \texttt{\_id}-sorted \texttt{find}),
  reference targets by single \texttt{findOne}s; official Rust
  driver, sync API. Read-only. Target: a standard connection
  string with the database as the path,
  \texttt{mongodb://HOST[:PORT]/DATABASE[?options]} (or
  \texttt{mongodb+srv://}); a missing
  \texttt{serverSelectionTimeoutMS} defaults to 5\,s. \\
\texttt{quarb-kubernetes} & The Kubernetes API, which is
  already a tree: the root holds one listing per discovered
  resource (cluster-scoped kinds plus the all-namespaces view,
  kubectl's \texttt{-A}); each namespace object carries its
  scoped listings beside its own fields; objects descend as
  their JSON, RFC\,3339 strings as instants.
  \texttt{ownerReferences} are native references
  (\texttt{::owner\textasciitilde>} climbs Pod → ReplicaSet →
  Deployment schema-free; \texttt{->node},
  \texttt{::namespace\textasciitilde>}); unknown property names
  fall through to \texttt{metadata.labels}. Lazy per listing,
  discovery at connect. Transport is \texttt{kubectl get --raw}
  over subprocess (git-adapter posture, zero new dependencies;
  \texttt{QUARB\_KUBECTL} overrides the binary), so contexts and
  auth behave exactly as kubectl does. Read-only. Target:
  \texttt{k8s:} / \texttt{k8s:CONTEXT} (alias
  \texttt{kubernetes:}). \\
\texttt{quarb-gcplogs} & Google Cloud Logging as a bounded
  snapshot — never a tail: the target must carry
  \texttt{since=} (a gcloud freshness duration or an ISO
  instant) and/or \texttt{limit=}, and an unbounded mount is
  refused. Entries oldest-first at the root as \texttt{/entry}
  children (the \texttt{/row} convention); severity as text
  property, lowercased trait
  (\texttt{/entry<error>}), and ordered numeric rank
  (\texttt{::::severity}, GCP's enum values); timestamps as
  typed instants; \texttt{::trace} the bare trace id — the
  \texttt{<=>} join key across services. Unknown property
  names fall through payload top level → labels → resource
  labels → httpRequest. \texttt{filter=} passes a raw Cloud
  Logging filter to the server (the mount-side cut);
  \texttt{until=} joins it as a timestamp clause. Transport is
  \texttt{gcloud logging read --format=json} over subprocess
  (kubectl posture; \texttt{QUARB\_GCLOUD} overrides —
  the bottled test fixture's hook; \texttt{account=} picks the
  gcloud account). Read-only. Target:
  \texttt{gcl:PROJECT?since=1h\&filter=…\&limit=N} (alias
  \texttt{gcplogs:}). \\
\texttt{quarb-cwlogs} & AWS CloudWatch Logs, the gcplogs
  design sheet over SigV4 (shared \texttt{quarb-aws} signing,
  credential chain, region resolution): bounded snapshots
  (\texttt{since=}/\texttt{limit=} mandatory), log groups as
  short-named root children with lazily fetched \texttt{/entry}
  events (or one group's entries at the root when the target
  names it), JSON messages decoded with property fallthrough
  (\texttt{::level}, \texttt{::requestId} — the cross-group
  join key), \texttt{filter=} as a server-side CloudWatch
  filter pattern, \texttt{::::group}/\texttt{::::stream}/
  \texttt{::::received}/\texttt{::::id} metadata.
  \texttt{endpoint=} overrides the URL (LocalStack; the
  bottled \texttt{cwlogs-server.py} fixture). Read-only.
  Target: \texttt{cwl:[GROUP]?since=1h\&filter=…\&limit=N}
  (alias \texttt{cloudwatch:}). \\
\texttt{quarb-azlogs} & Azure Monitor Logs through the Log
  Analytics KQL endpoint — the adapter composes the KQL itself
  (bounded table scan, \texttt{filter=} as a \texttt{| where}
  clause, \texttt{take} from \texttt{limit=}); \texttt{table=}
  and a bound are mandatory. One table's rows at the root as
  \texttt{/entry}, a comma list as root children per table.
  Columns are properties (\texttt{datetime} as instants,
  \texttt{dynamic} decoded into subtrees with fallthrough);
  \texttt{::OperationId} is the correlation key; the default
  value is \texttt{Message} or the \texttt{Name}+
  \texttt{ResultCode} request line. Bearer token from
  \texttt{\$AZURE\_LOG\_TOKEN} or the az CLI
  (\texttt{QUARB\_AZ}); \texttt{endpoint=} overrides (the
  bottled \texttt{azlogs-server.py}). Read-only. Target:
  \texttt{azl:WORKSPACE?table=…\&since=1h\&limit=N} (alias
  \texttt{azlogs:}). \\
\texttt{quarb-cflogs} & Cloudflare edge logs, Logpull-shaped:
  \texttt{GET zones/\{zone\}/logs/received} NDJSON, one flat
  record per request, fields as properties under their own
  names plus minted \texttt{::ray} and \texttt{::timestamp};
  the default value is the request line; \texttt{::::cache} the
  cache status. \texttt{since=} is mandatory (Logpull demands a
  range; \texttt{until} clamps a minute behind now). The Ray ID
  is what the origin sees as \texttt{cf-ray}, so mounting the
  edge next to origin logs makes request reconstruction
  cross-provider — one \texttt{<=>} on the ray, another on the
  origin's own trace. \texttt{\$CLOUDFLARE\_API\_TOKEN};
  \texttt{endpoint=} replays Logpush archives (the bottled
  \texttt{cflogs-server.py}). Read-only. Target:
  \texttt{cfl:ZONE\_ID?since=1h\&limit=N} (alias
  \texttt{cflogs:}). \\
\texttt{quarb-ddlogs} & Datadog Logs through the v2 search API
  (\texttt{POST /api/v2/logs/events/search}, cursor-paginated):
  bounded snapshots (\texttt{since=}/\texttt{limit=} mandatory),
  \texttt{query=} as Datadog's own search syntax run
  server-side. Severity is Datadog's \texttt{::status} plus the
  family trait and ordered \texttt{::::severity} rank;
  \texttt{::trace} is minted from \texttt{dd.trace\_id}; tags
  split into fallthrough properties (\texttt{env:prod} answers
  \texttt{::env}); custom attributes navigate as children. Auth
  \texttt{\$DD\_API\_KEY}/\texttt{\$DD\_APP\_KEY}, site from
  \texttt{site=}/\texttt{\$DD\_SITE}; \texttt{endpoint=}
  overrides (the bottled \texttt{ddlogs-server.py}). Read-only.
  Target: \texttt{ddl:?since=1h\&query=…\&limit=N} (alias
  \texttt{datadog:}). \\
\texttt{quarb-github} & GitHub, three shapes in their native
  registers. The tree mirrors github.com URLs: users/orgs at
  the root (addressed by login — the root does not enumerate),
  repositories as their children, \texttt{files} /
  \texttt{issues} / \texttt{pulls} / \texttt{releases} sections
  below; file content and issue bodies are node values. The
  social graph is edge fabric, API-backed both ways:
  \texttt{follows}, \texttt{starred}, \texttt{owner},
  \texttt{parent} (\texttt{<-parent} = the forks),
  \texttt{author}/\texttt{assignee}, \texttt{org}, and
  \texttt{contributor} — the first adapter with true
  \texttt{backlinks}, and its contributor edge carries data
  (\texttt{\$-::contributions}). Counts and scalars are
  properties (\texttt{::stars}, timestamps as instants); flags,
  issue labels, and repo topics are traits (\texttt{<fork>},
  \texttt{<bug>}, \texttt{<cli>}); an organization answers the
  \texttt{<org>} shorthand and the forge-neutral
  \texttt{<group>} alias. Direct addressing skips
  enumeration (\texttt{/rust-lang/rust/issues/12345}, closed
  included; listings show the API's default open set).
  Transport is \texttt{gh api} over subprocess
  (\texttt{QUARB\_GH} overrides); lazy, cached, read-only.
  Target: \texttt{github:[OWNER[/REPO]]}. \\
\texttt{quarb-gitlab} & GitLab, whose defining shape is the
  \emph{deep tree}: groups nest arbitrarily and mix subgroups
  with projects as children, which the REST API walks by
  URL-encoded full paths and the arbor restores to a path —
  \texttt{/tesslab/instruments/gauge/issues/7}, one GET per
  segment. Project sections: \texttt{files} (content as value),
  \texttt{issues} / \texttt{mrs} (the open sets; direct
  addressing by iid reaches any state), \texttt{releases},
  \texttt{pipelines} (jobs as children; statuses as traits —
  CI is half of what a GitLab project is). Code over social:
  no follows, stars as a count; the edge fabric is
  \texttt{member} (with access-level edge data —
  \texttt{\$-::access}, \texttt{\$-::role}), \texttt{parent} on
  forks (\texttt{<-parent} = the forks), and \texttt{author} /
  \texttt{assignee} / \texttt{reviewer}. Labels (scoped
  included), topics, visibility, and statuses are traits; a
  project answers the forge-neutral \texttt{<repo>} alias
  (\texttt{//*<repo>} sweeps either forge identically).
  Transport is \texttt{glab api} over subprocess
  (\texttt{QUARB\_GLAB} overrides; hosts via glab config), so
  self-managed instances work as glab does. Read-only. Target:
  \texttt{gitlab:[PATH]}. \\
\texttt{quarb-git} & The git repository driver: the object
  ...
  Read-only. \\
\texttt{quarb-atrep} & The atrep semantic-document adapter —
  the OQL homecoming. Mounts a deltos (\texttt{.atd}) or kanon
  (\texttt{.atk}) \emph{through its dialektos}: sims named by
  their dialect names (\texttt{//section}, \texttt{//footnote}),
  genoses as traits, \texttt{::onym}/\texttt{::taxis}/
  \texttt{::lemma} projections, deixes and onym-monosims as
  typed crosslinks with a precomputed reverse index (the first
  adapter to prebuild both \texttt{links} and
  \texttt{backlinks}), \texttt{<dangling>} as a
  broken-cross-reference linter. Mounting runs
  \texttt{kanonizo::tasso} (ordering only — onyms stay as
  authored; transclusion stays visible as edges). Depends on
  \texttt{quarb} and \texttt{atrep}. \\
...
\begin{enumerate}
  ...
  \item \textbf{Partial pushdown}
    ...
    metadata (each would observe the filtering). Since the
    JSON-column round (2026-07-31), refused predicates in the
    leading run degrade to \emph{superset prefilters} instead
    of aborting the plan: fixed-path JSON comparisons emit
    per-dialect forms proven never to drop a matching row
    (SQLite typed extract with a \texttt{json\_type='text'}
    escape hatch, PostgreSQL \texttt{jsonb\_typeof} CASE with
    \texttt{::float8}; existence and \texttt{!=} floor at
    \texttt{json\_type IS NOT NULL}) — extras are re-checked
    by the re-run, and a server-rejected prefilter surfaces
    through an eager prefetch and falls back to the scan
    rather than answering empty. Full JSON string-equality
    pushdown carries the dialect's validity guard
    (\texttt{json\_valid} and kin) and refuses
    numeric-looking literals (Quarb's \texttt{value\_eq}
    coerces those numerically; no extractor matches that).
    Measured: unpushable grouping over a 1\%-selective filter,
    2.31\,s vs.\ 0.081\,s; JSON numeric compare over 200k
    rows, 1.46\,s scanned vs.\ 0.25\,s prefiltered.
...
\texttt{--defs}, \texttt{--expand}
  & \live fragments: parse-time expansion with call-by-name
    \texttt{\$param} forms, defs files (\texttt{\#} line comments
    stripped in \texttt{parse\_defs}), and the unparser
    (\texttt{quarb::unparse}) rendering any parsed query back to
    canonical text (\texttt{--expand} = macroexpand); correlation
    chains and inline-pipe splices as bodies;
    \texttt{rec}/\texttt{group} field-name collisions are parse
    errors (\texttt{validate\_record\_convention}) \\
\texttt{| [::x =\textasciitilde{} /(\textbackslash{}w+)/] | %(k = \$1)},
\texttt{quai}
  & \live match captures (filter-stage \texttt{=\textasciitilde}
    ...
    \texttt{:show}/\texttt{:undo}; retired from \texttt{qua} in
    favor of the standalone \texttt{quai} crate;
    \texttt{NAME=TARGET} mount aliases via
    \texttt{quarb\_session::MountSpec} and in-session
    \texttt{:mount} via \texttt{Session::set\_executor} ---
    in-process mode only; the Python \texttt{Session}/kernel
    accrete defs-only cells through the \texttt{parse\_defs}
    binding) \\
...
\texttt{| \%(\$\$1/name::; /amt::)}
  & \live correlated subqueries: \texttt{\$\$} steps a capsa-scope
    ...
    \texttt{\$\$\_}, \texttt{\$\$ord}; the node form
    \texttt{\$\$::prop} / \texttt{\$\$/child::x} re-anchors on the
    invoking capsa's node; stacking \texttt{\$\$\$} steps further)
    — wide crosstabs, groupwise transforms; and the correlation
    keeps its \emph{witness}, so \texttt{\$*k} in pipeline stages
    projects the joined side into output (\texttt{SELECT a.x, b.y});
    the witness travels into every path a stage's operand or pipe
    hop navigates, so a nested step predicate
    (\texttt{(\^{}/c/*[::b\_id = \$\$1::id])}) reads the same
    thread's binding rather than re-searching the trace
    (2026-08-18) \\
\texttt{A <=> B[/k:: = \$\$/k:: and …]}
  & \live correlation, \emph{driver-first} (2026-07-25): the first
    expression drives (SQL's \texttt{FROM}); each joined
    expression carries its ON clause in its final step's brackets,
    reaching the driver as \texttt{\$\$…} and earlier entries as
    \texttt{\$*k}; \texttt{<=>?}\ marks the expression it precedes
    nullable (\texttt{LEFT JOIN}); one tuple binds the whole
    predicate set, searched depth-first with per-entry pruning,
    and \texttt{\$*N} may navigate from its context node
    (\texttt{\$\$1/field::}), so joins work where keys are
    children, e.g. JSON. Misplaced correlated references are
    parse errors, never silent nulls \\
...
pub trait AstAdapter {
    ...
    // --- the reference surface (defaulted) ---

    /// Resolve a cross-reference: `::prop-->hint` maps a property
    /// value to its in-document target (html: a `#fragment` href
    /// to the element with that id).
    fn resolve(&self, _n: NodeId, _prop: &str, _hint: Option<&str>)
        -> Option<NodeId> { None }

    /// The external reference a property holds -- an
    /// adapter-defined identifier of a document outside the arbor
    /// (html: the absolute URL, relative hrefs joined against the
    /// document's declared URL). On an in-document miss the engine
    /// consults the mount layer's reference targets (identifier ->
    /// mounted root) and lands, or records the identifier among
    /// the run's unresolved external references for the host's
    /// acquisition loop. The engine performs no IO, ever.
    fn external_ref(&self, _n: NodeId, _prop: &str, _hint: Option<&str>)
        -> Option<String> { None }

    /// The element `fragment` names inside `node`'s document --
    /// the landing rung of a fragment-carrying external reference.
    fn resolve_fragment(&self, _n: NodeId, _frag: &str)
        -> Option<NodeId> { None }

    /// The property carrying the node's own reference, for the
    /// bare arrow `//a-->` (html: an anchor's href); and the
    /// relation an edge carries (html: `rel`), the label falling
    /// back to the property name.
    fn ref_property(&self, _n: NodeId) -> Option<String> { None }
    fn ref_label(&self, _n: NodeId, _prop: &str) -> Option<String> { None }
}
\end{lstlisting}
...
\item \textbf{New trait methods need their forwarders.} The run
  path wraps every adapter in decorators (\texttt{WithNow},
  \texttt{AllowShell}), the session in \texttt{Dyn}, mounts in
  \texttt{MountAdapter}/\texttt{Shared}, models in
  \texttt{ModelAdapter} -- a defaulted method silently answers
  \texttt{None} through any wrapper that does not forward it.
  When adding to the trait, sweep every delegating impl.
