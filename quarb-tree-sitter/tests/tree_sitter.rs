...
fn rust_functions_fields_and_lines() {
    ...
    let a = TreeSitterAdapter::parse(src, "rs").unwrap();
...
fn deeply_nested_does_not_overflow() {
    ...
    let a = TreeSitterAdapter::parse(&src, "js").unwrap();
...
"#;
    let a = quarb_tree_sitter::TreeSitterAdapter::parse(src, "c").unwrap();
    ...
    assert!(quarb_tree_sitter::supported("c"));
    assert!(quarb_tree_sitter::supported("h"));
}
